...
void loop() {
  for (int x=0; x<12; x++) {
    ...
    if (currentInput[x] == HIGH && lastInput[x] == LOW) MIDI.sendNoteOn(48+x, velocity, channel);
    if (currentInput[x] == LOW && lastInput[x] == HIGH) MIDI.sendNoteOff(48+x, 0, 1);
  ...
  if ((currentValue[x] > lastValue[x] + 7) || (currentValue[x] < lastValue[x] - 7)) {
    ...
    MIDI.sendControlChange(20+x, currentValue[x], 1);
  }
  }
}
